...
class featurepipeline:
    ...
    def __init__(self, chars='1000', shuffling = True):
        import pandas as pd
        import numpy as np
        import sys
        sys.path.append('../')
        ...
        # read text sections
        Y_test = pd.read_pickle('../develop/Y_test_'+chars).values
        Y_train = pd.read_pickle("../develop/Y_train_"+chars).values
        X_test = pd.read_pickle("../develop/X_test_"+chars).values
        X_train = pd.read_pickle("../develop/X_train_"+chars).values
        self.chars = chars
    ...
    def processText(self, preprocessor, tokenizer, ngram, max_features, max_df, min_df, n_iter, n_comp, normalizer, balance = None):
        ...
        stopWords180517 = ["mutations", "patients", "mutants", "cells", "variants"] + list(stopWords)
