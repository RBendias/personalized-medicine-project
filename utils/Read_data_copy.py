...
def read_data(date, stack = False):
    '''
    ...
    stage2 = Path("/mnt/4_TB_HD/ramona/utils/features/stage2_"+date)
    if stage2.is_file():
        stage2_transf = pd.read_pickle(stage2)
        ...
        stage2_transf, stage2_transf_filtered = self.createstage2sets(self, date)
...
def createstage2sets(date, chars):
    model = pd.read_pickle('/mnt/4_TB_HD/ramona/utils/textPipelines/' + date)
    print(model.named_steps)
    stage2 = pd.read_pickle('/mnt/4_TB_HD/ramona/utils/stage2_data/stage2test_' + chars + '.sav')
    return stage2, model